...
export default async function PortfolioDetailPage({
  ...
  const { slug } = await params;
  const [project, services, portfolioProjects] = await Promise.all([
  ...
  const [industry, breadcrumb] = await Promise.all([
    getIndustryBySlug(project.industry),
    breadcrumbSchema([
      { name: "Home", url: "/" },
  ...
  return (
    <>
      ...
      <section className={`relative overflow-hidden bg-gradient-to-br ${getGradient(project.gradientKey)} pb-20 pt-24 sm:pt-32`}>
