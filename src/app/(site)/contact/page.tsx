...
export default async function ContactPage() {
  const [siteConfig, services, breadcrumb] = await Promise.all([
    getSiteConfig(),
    getServices(),
    breadcrumbSchema([
      { name: "Home", url: "/" },
  ...
  // The consultation form's service dropdown is populated from the database.
    ...
    <>
      ...
      <section id="consultation" className="scroll-mt-28 section-pad bg-muted/30">
        <div className="container-brand grid grid-cols-1 gap-14 lg:grid-cols-[1fr_1.3fr]">
          ...
          <Reveal delay={0.1} className="rounded-3xl border border-border bg-card p-6 sm:p-8">
            <ConsultationForm services={serviceOptions} />
          </Reveal>
        </div>
      </section>
