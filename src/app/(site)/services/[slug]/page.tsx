...
import { getServiceBySlug, getServices } from "@/lib/db/content";
import { getSiteConfig } from "@/lib/db/settings";
import { getIcon } from "@/lib/icons";

/**
 * With no reachable database — as in CI, where `npm run build` runs without
 * DATABASE_URL — `getServices()` returns an empty list and nothing is
 * prerendered. `dynamicParams` then lets pages render on demand at runtime,
 * so a missing database degrades the build instead of failing it.
 */
...
export async function generateMetadata({
  ...
  const [service, siteConfig] = await Promise.all([getServiceBySlug(slug), getSiteConfig()]);
  if (!service) return {};
...
export default async function ServiceDetailPage({
  ...
  const [breadcrumb, serviceLd] = await Promise.all([
    breadcrumbSchema([
      { name: "Home", url: "/" },
